...
describe('VehicleService.createVehicle', () => {
  it('cria veículo quando placa não existe e owner existe', async () => {
    ...
    repo.create.mockImplementation(async (entity) => makeVehicle({ ...entity.toJSON(), id: 99 }));
  ...
  it('erro se placa existente', async () => {
    ...
    await expect(() => sut.createVehicle({
      idPlate: 'ABC1234', type: 'CAR', model: 'X', brand: 'Y', manufactureYear: 2020, modelYear: 2021, color: 'Blue', ownerId: 10
  ...
  it('erro se owner não encontrado', async () => {
    ...
    await expect(() => sut.createVehicle({
      idPlate: 'NEW1111', type: 'CAR', model: 'Z', brand: 'Brand', manufactureYear: 2020, modelYear: 2021, color: 'Red', ownerId: 999
...
describe('VehicleService.updateVehicle', () => {
  it('atualiza veículo existente', async () => {
    ...
    repo.update.mockImplementation(async (_id, partial) => makeVehicle({ ...existing.toJSON(), ...partial }));
  ...
  it('erro se veículo não encontrado', async () => {
    ...
    await expect(() => sut.updateVehicle(1, { color: 'Purple' })).rejects.toBeInstanceOf(NotFoundServerException);
  });
});
...
describe('VehicleService.deleteVehicle', () => {
  ...
  it('erro se veículo não existe', async () => {
    ...
    await expect(() => sut.deleteVehicle(1)).rejects.toBeInstanceOf(NotFoundServerException);
  });
});
...
describe('VehicleService.findById', () => {
  ...
  it('erro not found', async () => {
    ...
    await expect(() => sut.findById(5)).rejects.toBeInstanceOf(NotFoundServerException);
  });
});
...
describe('VehicleService.list & countAll', () => {
  it('lista veículos', async () => {
    const { sut, repo } = makeSut();
    repo.list.mockResolvedValue([makeVehicle({ id: 1 }), makeVehicle({ id: 2, idPlate: 'DEF5678' })]);
