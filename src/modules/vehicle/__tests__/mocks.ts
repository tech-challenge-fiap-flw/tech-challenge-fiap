...
export const makeVehicle = (overrides: Partial<ReturnType<VehicleEntity['toJSON']>> = {}) => {
  const base = {
    id: overrides.id ?? 1,
    idPlate: 'ABC1234',
    type: 'CAR',
  ...
  return VehicleEntity.restore({ ...base, ...overrides });
};
...
export const userServiceMock = (): jest.Mocked<IUserService> => ({
  ...
  updateUser: jest.fn(),
  deleteUser: jest.fn(),
  findById: jest.fn(async (id: number) => ({ id, name: 'Owner', email: 'owner@mail.com', type: 'CLIENT', cpf: '000', cnpj: null, phone: '0', address: null, city: null, state: null, zipCode: null } as unknown as UserOutput)),
  findByEmail: jest.fn(),
  list: jest.fn(),
  countAll: jest.fn(),
});
...
export const userServiceNotFoundMock = (): jest.Mocked<IUserService> => ({
  ...
  updateUser: jest.fn(),
  deleteUser: jest.fn(),
  findById: jest.fn(async (_id: number) => { throw new NotFoundServerException('User not found'); }),
  findByEmail: jest.fn(),
  list: jest.fn(),
  countAll: jest.fn(),
});
