...
export class ServiceOrderService {
  ...
  async create(input: Omit<ReturnType<ServiceOrderEntity['toJSON']>, 'id' | 'creationDate' | 'currentStatus' | 'active'>) {
  ...
  async changeStatus(id: ServiceOrderId, newStatus: ServiceOrderStatus, userId: number) {
    ...
    if (!current) throw Object.assign(new Error('Service order not found'), { status: 404 });
    const before = current.toJSON().currentStatus;
    const updated = await this.repo.updateStatus(id, newStatus);
    await this.historyRepo.create(
      ServiceOrderHistoryEntity.create({ idServiceOrder: id, userId, oldStatus: before, newStatus })
    );
  ...
  async delete(id: ServiceOrderId) { await this.repo.softDelete(id); }
}
