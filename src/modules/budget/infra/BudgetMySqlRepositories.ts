...
export class BudgetMySqlRepository implements BudgetRepository {
  async create(entity: BudgetEntity): Promise<BudgetEntity> {
    ...
    const id = result.at(0)?.insertId as BudgetId;
  ...
  async update(id: BudgetId, partial: Partial<BudgetProps>): Promise<BudgetEntity> {
    ...
    await query<ResultSetHeader>(`UPDATE budgets SET ${setClause} WHERE id = ?`, params);
  ...
  async list(offset: number, limit: number): Promise<BudgetEntity[]> {
    const rows = await query<BudgetProps & RowDataPacket[]>(
      `SELECT * FROM budgets WHERE deletedAt IS NULL ORDER BY id LIMIT ? OFFSET ?`, 
      [limit, offset]
    );

    return rows.map((r) => BudgetEntity.restore(r));
  }

  async countAll(): Promise<number> {
    const rows = await query<{ c: number }>(`SELECT COUNT(*) AS c FROM budgets WHERE deletedAt IS NULL`);
...
export class BudgetVehiclePartMySqlRepository implements BudgetVehiclePartRepository {
  async add(entity: BudgetVehiclePartEntity): Promise<BudgetVehiclePartEntity> {
    ...
    const id = results.at(0)?.insertId as number;
  ...
  async listByBudget(budgetId: BudgetId): Promise<BudgetVehiclePartEntity[]> {
    const rows = await query<BudgetVehiclePartProps & RowDataPacket[]>(
      `SELECT * FROM budget_vehicle_parts WHERE budgetId = ?`, 
      [budgetId]
    );
    
    return rows.map((r) => BudgetVehiclePartEntity.restore(r));
  }
}
...
export class BudgetVehicleServiceMySqlRepository implements BudgetVehicleServiceRepository {
  ...
  async listByBudget(budgetId: BudgetId): Promise<BudgetVehicleServiceEntity[]> {
    const rows = await query<BudgetVehicleServiceProps & RowDataPacket[]>(
      `SELECT * FROM budget_vehicle_services WHERE budgetId = ?`, 
      [budgetId]
    );
    
    return rows.map((r) => BudgetVehicleServiceEntity.restore(r));
  }
}