...
describe('Diagnosis Controllers', () => {
  describe('CreateDiagnosisController', () => {
    it('cria diagnosis', async () => {
      ...
      const res = await controller.handle(
        makeReq({ description: 'Noise', vehicleId: 10 })
      );
    ...
    it('falha validação', async () => {
      ...
      await expect(
        controller.handle(makeReq({ description: 'No', vehicleId: -1 }))
  ...
  describe('UpdateDiagnosisController', () => {
    it('atualiza diagnosis', async () => {
      ...
      service.updateDiagnosis.mockResolvedValue({
        id: 2,
        description: 'Updated',
        vehicleId: 10,
        creationDate: new Date(),
      ...
      const res = await controller.handle(
        makeReq({ description: 'Updated' }, { id: '2' })
      );
    ...
    it('falha validação update', async () => {
      ...
      await expect(
        controller.handle(makeReq({ description: 'No' }, { id: '2' }))
      ).rejects.toBeInstanceOf(HttpError);
    });
  });
