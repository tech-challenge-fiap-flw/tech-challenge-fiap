...
export class DiagnosisMySqlRepository implements DiagnosisRepository {
  async create(entity: DiagnosisEntity): Promise<DiagnosisEntity> {
    ...
    const sql = `INSERT INTO diagnosis (description, creationDate, vehicleId, responsibleMechanicId, deletedAt)
                 VALUES (?, ?, ?, ?, ?)`;
    const params = [data.description, data.creationDate, data.vehicleId, data.responsibleMechanicId ?? null, data.deletedAt ?? null];
  ...
  async findById(id: DiagnosisId): Promise<DiagnosisEntity | null> {
    ...
    return DiagnosisEntity.restore(rows[0] as any);
  }
  async update(id: DiagnosisId, partial: Partial<DiagnosisProps>): Promise<DiagnosisEntity> {
    ...
    if (keys.length === 0) {
      ...
      if (!found) throw Object.assign(new Error('Diagnosis not found'), { status: 404 });
      return found;
    }
    const setClause = keys.map((k) => `${k} = ?`).join(', ');
    const params = keys.map((k) => (partial as any)[k]);
    params.push(id);
    await query<ResultSetHeader>(`UPDATE diagnosis SET ${setClause} WHERE id = ?`, params as any);
    const updated = await this.findById(id);
    if (!updated) throw Object.assign(new Error('Diagnosis not found'), { status: 404 });
    return updated;
  }
  ...
  async list(offset: number, limit: number): Promise<DiagnosisEntity[]> {
    const rows = await query<DiagnosisProps & RowDataPacket>(`SELECT * FROM diagnosis WHERE deletedAt IS NULL ORDER BY id LIMIT ? OFFSET ?`, [limit, offset]);
    return rows.map((r: any) => DiagnosisEntity.restore(r));
  }

  async countAll(): Promise<number> {
    const rows = await query<RowDataPacket & { c: number }>(`SELECT COUNT(*) AS c FROM diagnosis WHERE deletedAt IS NULL`);
